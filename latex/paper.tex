...
We demonstrate the scalability of our parallel algorithm in terms of data size and number of processors, and apply the hierarchical clustering approach to large (dense) hyperspectral imaging and (sparse) text document data. \GB{On 40 nodes of Summit supercomputer, our algorithm achieves (a) linear speedup for solving the Rank-2 Non-negative Least squares kernels and (b) achieves 14x speedup on synthetic matrices. We also demonstrate that in the case of real world matrices, the algorithm scales to 80 summit nodes.} 
\end{abstract}
