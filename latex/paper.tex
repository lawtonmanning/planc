...
Analysis of the algorithm shows the dependence of execution time on computation and communication costs as well as the number of clusters computed.

We demonstrate in \Cref{sec:perf} the efficiency and scalability of our parallel algorithm on three data sets, including the HSI data of the DC mall and an image classification data set involving skin melanoma.
...
The image classification data set requires 800 GB of memory across multiple nodes to process, and in scaling from 10 nodes to 80 nodes of the Summit supercomputer, we demonstrate parallel speedups of 7.1$\times$ for a single Rank-2 NMF and ZZ$\times$ for a complete hierarchical clustering.
...
As explained in more detail in \Cref{sec:r2nmf}, the optimal solution across the 4 possibilities \LM{sets} can be determined efficiently to solve the NNLS subproblem more quickly than general-rank approaches like block principal pivoting.
