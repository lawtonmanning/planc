...
\GB{specify that $\imath=\jmath=100$ in all experiments, also what do we set $k$ to be?  fixed number for each dataset?  any other experimental parameters we should specify?}
...
\GB{4 plots: 2 data sets with speedup/breakdown}

\begin{itemize}
	\item theory says...
	\item Figs 3 and 5 show performance relative to the smallest number of nodes required to store data and factor matrices
	\item strong scaling is nearly linear, see 49X (check number) on 50 nodes for synthetic data and 7.3X (check number) on 80 nodes for image data
	\item Figs 4 and 6 show time breakdown, explain scaling: MM scales perfectly because it's mem-bw bound and the local problem size decreases linearly, bandwidth cost is fixed relative to $p$, so relative cost grows as we scale; comm is bandwidth bound because we see all-gather and reduce-scatter dominate all-reduce
\end{itemize}
