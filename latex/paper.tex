...
\GB{use Kuang's related work for NMF (2016 paper for divide-and-conquer NMF in JGO: \url{https://link.springer.com/article/10.1007/s10898-017-0515-z}), use PPoPP/TKDE/arXiv of ours and also Liavas, pyramid NMF: \doi{10.1109/IJCNN.2003.1224004}}

\subsection{NMF}

\GB{general NMF, BCD/AO algorithms, NNLS}

\subsection{Parallel NMF}

\GB{PLANC and other related work}
...
\begin{equation}
\label{eq:r2nmfcost}
\gamma \cdot O\left( \frac{\imath mn}{p} \right) + \beta \cdot O(\imath n) + \alpha \cdot O(\imath \log p).
\end{equation}
...
\GB{track NNLS of $\M{W}$ term even though it's lower order}
...
\begin{equation*}
\label{eq:nodecost}
\gamma \cdot O\left( \frac{(\imath+\jmath) m\overline{n}}{p} \right) + \beta \cdot O((\imath+\jmath) \overline{n}) + \alpha \cdot O((\imath+\jmath) \log p).
\end{equation*}
Because the sum of the number of columns across any level of the tree is $n$, the cost of the $\ell$th level of the tree is
\begin{equation*}
\label{eq:levelcost}
\gamma \cdot O\left( \frac{(\imath+\jmath) mn}{p} \right) + \beta \cdot O((\imath+\jmath) n) + \alpha \cdot O((\imath+\jmath) 2^\ell \log p).
\end{equation*}
...
\label{eq:treecost}
\begin{split}
\gamma \cdot O\left( \frac{(\imath+\jmath) mn}{p} \log k \right) + \beta \cdot O((\imath+\jmath) n \log k) \\ + \alpha \cdot O((\imath+\jmath) k \log p).
\end{split}
\end{equation*}

We see that the computational cost is logarithmic in $k$ and perfectly load balanced.
