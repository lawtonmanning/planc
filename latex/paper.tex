...
Our data distribution and parallelization strategies are designed to maintain computational load balance throughout the data-dependent hierarchy of computation while limiting interprocess communication, allowing the algorithm to scale to large dense data sets.
We demonstrate the scalability of our parallel algorithm in terms of data size and number of processors, and apply the hierarchical clustering approach to hyperspectral imaging and image classification data.
\end{abstract}
...
Nonnegative Matrix Factorization (NMF) has been demonstrated to be an effective tool for unsupervised learning problems including clustering \cite{}. \GB{add citations}
...
The first bipartitioning separates vegetation (cluster 1) from non-vegetation (cluster 2), the bipartitioning of cluster 1 seems to separate grass (cluster 3) from trees (cluster 4), the bipartitioning of cluster 2 seems to separate buildings (cluster 5) from sidewalks/water (cluster 6), and so on.

\begin{itemize}
	\item discuss limitation of current implementations of the hierarchical method in MATLAB and shared-memory parallelization in terms of scalability to data size and bandwidth limitations
	\item brief summary of parallelization approach, point to analysis
	\item summarize results, highlight 600 GB data set and scaling to 2000-2500 cores, speedups of up to X for rank-2 NMF and up to Y for full hierarchical clustering algorithm
\end{itemize}
