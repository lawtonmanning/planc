...
Assuming a balanced tree and relatively small $k$, \Cref{eq:treecost} shows that the dominant computational cost is proportional to $\log k$, while flat NMF algorithms have a dominant cost that is linear in $k$ \cite{KBP17}.
\Cref{fig:rankscaling} shows the raw time for various values of $k$, confirming that running time for HierNMF grows more slowly in $k$ than a flat NMF algorithm from PLANC \cite{EH+19-TR} with the same number of columns and processor grid.
